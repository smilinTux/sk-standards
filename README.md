...
| [**SKWORLD_MODULE_CONTRACT_STANDARD**](./standards/SKWORLD_MODULE_CONTRACT_STANDARD.md) | The frozen SKWorld subapp contract (manifest schema **v1.1** + `skworld_module_api` **v0**): ONE capauth-signed `skworld.module.json` per subapp with **two facets** — a **UI facet** (id · name · grade A/B · entry · nav · deeplinkPrefix · auth audience/scopes · memory · health) the Dart shell mounts via `SkworldModule`/`ShellContext?`/`AuthContext`/`ShellBus` (the **`shell == null` = standalone** rule), and an **operator facet** (`contractVersion` · cli · repos · conditions · proposedStandardActions) Atlas observes and acts on. Plus the **registry + signing rule** (`~/.skcapstone/shell/modules`, local-file or `/.well-known` URL, detached capauth signature must verify before mount). JSON Schema + documented example in [`reference/skworld-module/`](./reference/skworld-module/). |
