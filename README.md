...
| [**SKWORLD_MODULE_CONTRACT_STANDARD**](./standards/SKWORLD_MODULE_CONTRACT_STANDARD.md) | The SKWorld subapp contract (manifest schema **v1.2** + `skworld_module_api` **v0**): ONE capauth-signed `skworld.module.json` per subapp with **two required facets** — a **UI facet** (id · name · grade A/B · entry · nav · deeplinkPrefix · auth audience/scopes · memory · health) the Dart shell mounts via `SkworldModule`/`ShellContext?`/`AuthContext`/`ShellBus` (the **`shell == null` = standalone** rule), and an **operator facet** (`contractVersion` · cli · repos · conditions · proposedStandardActions) Atlas observes and acts on — plus (v1.2, both **optional**, strict superset of v1.1) an **install facet** (a pluggable capability pack's ordered typed steps: `sql_migration` · `db_roles` · `content_repo` · `seed` · `fleet_objects` · `doctor`) the skos installer runs, and a **knowledge facet** (namespace · search_fn · retriever · reader_role · graph) the Atlas seat auto-enables for RAG. Plus the **registry + signing rule** (`~/.skcapstone/shell/modules`, local-file or `/.well-known` URL, detached capauth signature must verify before mount, and it is the trust gate for operator discovery too). JSON Schema + two documented examples (UI+operator, and the skbrain capability pack) in [`reference/skworld-module/`](./reference/skworld-module/). |
| [**MCP_TOOL_OWNERSHIP_STANDARD**](./standards/MCP_TOOL_OWNERSHIP_STANDARD.md) | **One owner repo per MCP tool name.** When the same tool is exposed by more than one `sk*` MCP server (skcapstone / skchat / skmemory), the owning repo (by domain: messaging→skchat, memory→skmemory, telegram-bridge→skcapstone, coord/ITIL→skcoord) holds the canonical implementation; other servers may re-expose it only as a **thin delegate** (same schema, never a second copy of the logic) or **drop** it. Includes the 2026-08-06 inventory (192 distinct tools, 13 duplicated) with per-tool ownership + disposition, the domain-assignment rule for new tools, and the per-repo compliance checklist. |
| [**SKWORLD_AUTHORIZATION_STANDARD**](./standards/SKWORLD_AUTHORIZATION_STANDARD.md) | **One PDP, many thin PEPs.** How every authenticated subapp (skchat / skdashboard / skboard / skcode / skos / skgateway) enforces authorization against the single `capauth.authz.decide` PDP: the `<subapp>.<action>` capability taxonomy (read/write/act -> TOFU/attested/verified), the **route-coverage discipline** (every gated route capability-mapped, explicit-public, or explicit-self-auth) with a **CI completeness gate** that makes an enforce flip provably safe (shadow soak is necessary but NOT sufficient, it structurally cannot see unmapped routes), method-aware mapping, the self-auth registry, declared per-subject grant bundles audited via `decide()`, the shadow->coverage-green->grants->enforce rollout rails (RCE surfaces deny-all-until-review), and an optional `authz` manifest facet (contract v1.3). Distilled from the two CR-3 enforce incidents. |
