...
4. **A real contract for the live coding fleet.** The pool controller for the pi fleet exists only as text typed into a live session (pack A.5, FACT), and the fleet's private provenance synonyms were not mapped to the engine's canonical twin-gate verdict and RunRecord content hash concepts (pack A.6, FACT). Section 4.
...
3. **The provenance vocabulary.** The fleet's private synonyms map onto the engine's canonical twin-gate verdict and `RunRecord` content hash concepts, then retire. One vocabulary, defined where the code is, is used by both lanes' briefs. The arena subsystem's content-addressing stays "proposed" status and is not on this path (pack A.9).
...
The `RunRecord` writer wired at the twin-gate verdict boundary for both lanes; lane 1 merges routed through `ratify()`; fleet brief templates rewritten to the canonical engine vocabulary, replacing private synonyms with twin-gate verdict and RunRecord content hash.
