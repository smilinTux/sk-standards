...
| 🔁 [**SKRSI**](https://github.com/smilinTux/skrsi) | **SK Recursive SELF Improvement**, where SELF is **Systematic Evaluation, Learning, and Feedback**. It measures software lifecycle evidence, evaluates bounded experiments, records lessons, and returns reviewed proposals to existing authorization gates. | Current runtime components live in `skcapstone`; aggregate visibility lives in `skdashboard`; `governs` no product-domain policy and owns no actuation authority. |
| ⚙️ [**skharness**](https://github.com/smilinTux/skharness) | The **autocode ENGINE** (Layer-0 execution) — one Harness, two planes: **task plane** = the autocode loop (assess→build→grade→twin-gate→merge, context-lean, sandboxed), **session plane** = `skcode-hostd` (host + steer live agent sessions, `:9394`). Context-lean: a build sandbox never sees the operator's personal graph. | `backs` skos autopilot (skos re-exports `skharness.autocode`) and the skcode client; `depends-on` capauth / skgateway / skcoord. Governed by [ADR-0001](./decisions/ADR-0001-skos-skharness-skcode-layering.md). |
| ⌨️ **skcode** | The **interactive coding client** — the human front door to the engine (watch/steer sessions, code by hand, dispatch). Ships distributed today: daemon in **skharness** (`skcode-hostd`, P0 read-only MVP), Flutter pane in **skchat-app** (`features/skcode`), operator adapter in **skcapstone**. Interactive dispatch is roadmap. No standalone package yet. | `drives` skharness (thin client, never a second engine); reconciled by [ADR-0001](./decisions/ADR-0001-skos-skharness-skcode-layering.md). |
