...
| `actor.id` | string | MUST | The authenticated identity of the writer, from `capauth.resolve_agent_identity()`, carried as the canonical fqid form defined in [IDENTITY_NAMING_STANDARD](./IDENTITY_NAMING_STANDARD.md) section 1. The `capauth:` wire form is a DEPRECATED alias per that standard section 2.1: the section 2.4 validator MUST strip it at ingest and it MUST NOT be stored in this field. NEVER free text, NEVER a magic string like `"human"` or `"operator"` typed by a caller. |
