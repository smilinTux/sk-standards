...
`sk-standards` is the canonical home of the SKWorld engineering standards: 23 standards
...
|---|---|---|
| The standards | `standards/*.md` | 23 canonical documents. Every `sk*` repo conforms to these. Indexed from `README.md`. |
...
| Validators | `scripts/` | Ten validator scripts, including docs, CI, fences, module schema, service units, actuation registry, readiness, action authorization, merge gate, and coding lanes. |
...
flowchart TD
    subgraph skstd["sk-standards (this repo, no runtime)"]
      README["README.md<br/>the hub: indexes all 23 standards"]:::doc
      STD["standards/*.md<br/>23 canonical standards"]:::doc
...
1. **`README.md`** - the hub. A one-line "what it governs" for each of the 23 standards,
...
checks:
  ...
  - name: the standards count claimed throughout this SOP still matches the tree
    run: test "$(ls standards/*.md | wc -l)" = 23
